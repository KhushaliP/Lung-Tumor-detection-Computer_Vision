...
def get_weighted_random_sampler(training_data, neg_weight, pos_weight):
    weighted_list = [pos_weight if mask.sum(
    ) > 0 else neg_weight for (_, mask) in training_data]
...
def train_model(input_args, encoder_channels, decoder_channels):
    ...
    aug_pipeline = iaa.Sequential([
        ...
        iaa.Affine(translate_percent=0.15, scale=(
            0.85, 1.15), rotate=(-45, 45)),
        iaa.ElasticTransformation()
    ])

    training_data = get_dataset(
        input_args.preprocessed_input_dir, aug_pipeline, data_type='train')
    validation_data = get_dataset(
        input_args.preprocessed_input_dir, data_type='val')
    neg_weight, pos_weight = get_class_balancing_weights(training_data)
    sampler = get_weighted_random_sampler(
        training_data, neg_weight, pos_weight)
    ...
    visualizations_dataloader = DataLoader(validation_data, batch_size=input_args.batch_size, num_workers=num_workers,
                                           shuffle=True)
    print(
        f"There are {len(training_data)} train images and {len(validation_data)} val images")

    segnet = SegNet(encoder_channels, decoder_channels,
                    input_args.num_classes, input_args.warm_start)
    ...
    trainer = pl.Trainer(gpus=gpus, logger=wandb_logger, log_every_n_steps=1,
                         ...
                         callbacks=[early_stopping_callback, images_saving_callback,
                                    learning_rate_callback], max_epochs=input_args.num_epochs)
...
def cli_main():
    ...
    parser.add_argument('--project_name', type=str,
                        default="lung-tumor-segmentation-segnet")
    parser.add_argument('--preprocessed_input_dir',
    ...
    train_model(args, encoder_channels=(3, 64, 128, 256, 512, 512),
                decoder_channels=(512, 512, 256, 128, 64, 64))
